...
// extractHistoryImages parses an OpenAI image response and returns only the
// URL-backed images (base64 is dropped so no multi-MB blob reaches the DB).
func extractHistoryImages(respBody []byte) []historyImage {
	...
	for _, d := range parsed.Data {
		...
		out = append(out, historyImage{Url: d.Url, RevisedPrompt: d.RevisedPrompt})
	}
	return out
}
