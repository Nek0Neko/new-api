...
export function GroupsList({ groups, selected, onSelect, onChanged }: Props) {
  ...
  return (
    <div className="flex flex-col gap-1 md:border-r md:pr-2">
      {groups.map((g) => (
        <button
          key={g.name}
          type="button"
          ...
          className={cn(
            'flex items-center justify-between gap-2 rounded px-2 py-1.5 text-left text-sm hover:bg-muted',
            selected === g.name && 'bg-muted font-medium'
          )}
        >
          ...
          <Badge
            variant={g.channel_count === 0 ? 'outline' : 'secondary'}
            className={cn(g.channel_count === 0 && 'text-amber-500')}
          >
            {g.channel_count}
          </Badge>
        </button>
