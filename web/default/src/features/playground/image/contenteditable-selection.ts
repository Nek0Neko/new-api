...
import { getSelectedTextMentionLabel } from './prompt-mentions'
...
function getContentEditableBoundaryOffset(
  ...
  if (!root.contains(container)) {
    // Handle selection boundaries outside the editor (e.g. Ctrl+A).
    const position = root.compareDocumentPosition(container)
    if (position & Node.DOCUMENT_POSITION_PRECEDING) return 0
    if (position & Node.DOCUMENT_POSITION_FOLLOWING)
      return root.textContent?.length ?? 0

    // If it is a parent container, use the offset to tell whether the boundary
    // is before or after the editor.
...
/** Get the plain-text caret offset inside a contentEditable. */
...
/** Place the caret at a given plain-text offset inside a contentEditable. */
export function setContentEditableCursor(el: HTMLElement, offset: number) {
  ...
  // If the offset overruns, place the caret at the end.
  if (node) {
    const range = document.createRange()
