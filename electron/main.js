...
let serverErrorLogs = [];
const PORT = 3000;
const DEV_FRONTEND_PORT = 9876; // Vite dev server port
...
function startServer() {
  return new Promise((resolve, reject) => {
    ...
    if (isDev) {
      ...
      console.log('  2. Frontend dev server: cd web && bun dev (port 9876)');
      console.log('');
      ...
      checkServerAvailability(DEV_FRONTEND_PORT)
        .then(() => {
          console.log('✓ Frontend dev server is accessible on port 9876');
          resolve();
        })
        .catch((err) => {
