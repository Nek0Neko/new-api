...
// GetUnfinishedImageTasks returns all not-yet-finished image tasks (platform=image).
// Used on startup so each task's playground history row can be marked failed
// before FailUnfinishedImageTasks flips the task records.
func GetUnfinishedImageTasks() ([]*Task, error) {
	var tasks []*Task
